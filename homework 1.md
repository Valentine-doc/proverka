...
Во вкладке HTTP отобразится ссылка. Копируем и вставляем ее в наш терминал через команду клонирования. 
Ждем загрузку. Далее через команду _git status_ можно увидеть, что этот репозиторий горит красным.
Для сохранения нам нужно использовать команду:
